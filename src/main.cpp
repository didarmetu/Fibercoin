...
#if defined(NDEBUG)
#error "Fibercoin cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 60 * 60 * 5; //5 hour
int64_t nReserveBalance = 0;

/** Fees smaller than this (in ufbc) are considered zero fee (for relaying and mining)
...
{
    RenameThread("fibercoin-scriptch");
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // FBC: We use certain sporks during IBD, so check to see if they are
